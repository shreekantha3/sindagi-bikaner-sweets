# TEST_REPORT — sindagi-bikaner-sweets

- `npm install`: PASS (87 packages)
- `npm run build`: PASS — `dist/index.html` 13.73 kB (gzip 3.63 kB), CSS 11.99 kB (gzip 3.25 kB), JS 1.20 kB (gzip 0.63 kB). Total `dist/` ≈ 36K.
- Base path: `/sindagi-bikaner-sweets/` ✓ (assets + favicon resolve under base in dist output)
...
- SEO: title + meta description + OG tags ✓; JSON-LD `CandyStore` (no `telephone`) ✓
- A11y/perf: skip link ✓; async Google Fonts (`media="print" onload`) + `<noscript>` fallback ✓; `prefers-reduced-motion` in CSS ✓; dark text (`text-gray-700`, `text-brand-700` #A63F0D) — no light-orange text ✓
